        ...
        def train(self):
            ...
            for idx, (layer) in enumerate(layers):
                if idx == 1:
                    ...
                    plt.text(
                        idx+bias, contrast_statistic_min[layer][0]-n_bias, '{}'.format(round(100*contrast_statistic_min[layer][0], 2)), fontsize=text)
                    ...
                    plt.text(
                        idx+bias_, contrast_statistic_max[layer][0]+p_bias, '{}'.format(round(100*contrast_statistic_max[layer][0], 2)), fontsize=text)
                    plt.text(
                        idx+bias, contrast_statistic_min[layer][0]-n_bias, '{}'.format(round(100*contrast_statistic_min[layer][0], 2)), fontsize=text)

            plt.bar(17, 0.9*contrast_statistic_max[layer][0]-0,
                    bottom=0, alpha=b_alpha, color='b')
            ...
            plt.xticks([i for i in range(1, 19)], ['{}'.format(i)
                       for i in range(1, 19)], fontsize=fontsize)
            plt.grid(axis='y')
            ...
            plt.savefig(
                '/home/jyl/distill-defense/preliminary/box_{}.png'.format(attack), bbox_inches='tight')
            plt.savefig(
                '/home/jyl/distill-defense/preliminary/box_{}.pdf'.format(attack), bbox_inches='tight')
