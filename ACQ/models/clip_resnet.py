...
class Clip_ReLU(nn.ReLU):
    def __init__(self, inplace: bool = False, clip_min=None, clip_max=None, bit=4):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        input = F.relu(input, inplace=self.inplace).clamp(
            clip_min, clip_max)
        return input

    def quantize(self, input: Tensor):
        S = self.clip_max - self.clip_min
        S /= math.pow(2, self.bit)
        q = torch.round(input/S).clamp_(0, 255)
        r = S*q
        return r
...
def CLIP_ResNet18(num_classes: int = 10, clip_max=None, clip_min=None, **kwargs):
    return _resnet(BasicBlock, [2, 2, 2, 2], num_classes, clip_max=clip_max, clip_min=clip_min, **kwargs)


def CLIP_ResNet34(num_classes: int = 10, **kwargs):
    return _resnet(BasicBlock, [3, 4, 6, 3], num_classes, **kwargs)


def CLIP_ResNet50(num_classes: int = 10, **kwargs):
